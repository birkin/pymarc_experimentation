...
class Extractor( object ):
    ...
    def extract_info():
        ...
        with open( big_marc_filepath, 'rb' ) as fh:
            ...
            for record in reader:
                ( fields, title, bib_id, item_id, record_dct_logged ) = self.setup_main_loop( record )
                for field_dct in fields:

                    # ( bib_id, item_id ) = self.find_bib_and_item( bib_id, item_id )
    ...
    def setup_main_loop( self, record ):
        ...
        return_tpl = ( fields, title, bib_id, item_id, record_dct_logged )
        log.debug( 'return_tpl ( fields, title, bib_id, item_id, record_dct_logged ), ```{}```'.format(return_tpl) )
        return return_tpl
